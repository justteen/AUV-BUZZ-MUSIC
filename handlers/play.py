...
async def play(_, message: Message):
    global que
    lel = await message.reply("🔄 **Processing Via ✯AUV-BUZZ MUSIC✯**")
    ...
    await lel.edit("🔎 **Finding Via ✯AUV-BUZZ MUSIC✯**")
    sender_id = message.from_user.id
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    await lel.edit("🎵 **Processing Via ✯AUV-BUZZ MUSIC✯**")
    ...
    if message.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        photo="final.png",
        reply_markup=keyboard,
        caption="▶️ **Playing** song requested by {} via AUV-BUZZ MUSIC ".format(
...
async def deezer(client: Client, message_: Message):
    global que
    lel = await message_.reply("🔄 **Processing Via ✯AUV-BUZZ MUSIC✯**")
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        appendable = [s_name, r_by, loc]
        qeue.append(appendable)
        await res.edit_text(f"✯AUV-BUZZ MUSIC✯= #️⃣ Queued at position {position}")
    else:
        await res.edit_text("✯AUV-BUZZ MUSIC✯=▶️ Playing.....")
...
async def jiosaavn(client: Client, message_: Message):
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        m = await client.send_photo(
            ...
            reply_markup=keyboard,
            photo="final.png",
            caption=f"✯AUV-BUZZ MUSIC✯=#️⃣ Queued at position {position}",
